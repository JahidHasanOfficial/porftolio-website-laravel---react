...
export default function About({ experiences, educations, certificates, resume, skills }) {
    ...
    return (
        <AppLayout>
            {/* Header banner */}
            <section className="relative bg-slate-950 py-20 border-b border-slate-800">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 text-xs font-mono font-medium uppercase tracking-wider">
            ...
            <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                            ...
                            <div className="relative border-l border-slate-800 pl-6 space-y-10">
                                {experiences && experiences.map((exp) => (
                                    <div key={exp.id} className="relative">
                                        <div className="absolute -left-[31px] top-1.5 h-4 w-4 rounded-full border-4 border-slate-950 bg-cyan-400" />
                                        <div className="space-y-2">
                                            ...
                                            <div className="text-sm text-slate-300 leading-relaxed space-y-1.5 pt-2">
                                            ...
                                            {exp.achievements && (
                                                <div className="mt-3 p-3.5 rounded-xl bg-slate-900 border border-slate-800 text-xs font-mono text-slate-300">
                                                    <span className="text-emerald-400 font-bold">Key Achievement: </span>
                                                    {exp.achievements}
                                                </div>
                                            )}
                                            ...
                                            {Array.isArray(exp.technologies) && (
                                                <div className="flex flex-wrap gap-1.5 pt-2">
                                                    {exp.technologies.map((tech, idx) => (
                                                        <span key={idx} className="text-[11px] font-mono bg-slate-900 text-slate-400 px-2.5 py-1 rounded border border-slate-800">
                                                            {tech}
                                                        </span>
                                                    ))}
                                                </div>
                                ...
                                <div className="relative border-l border-slate-800 pl-6 space-y-8">
                                    {educations.map((edu) => (
                                        <div key={edu.id} className="relative">
                                            <div className="absolute -left-[31px] top-1.5 h-4 w-4 rounded-full border-4 border-slate-950 bg-indigo-400" />
                                            <div className="space-y-1">
                                                <span className="text-xs font-mono text-slate-500 uppercase tracking-wider">
                                                    {edu.session}
                                                </span>
                                                <h3 className="text-lg font-bold text-white">{edu.degree}</h3>
                                                <p className="text-xs text-slate-400">{edu.institute} {edu.department && `(${edu.department})`}</p>
                                                {edu.result && (
                                                    <p className="text-xs font-mono text-cyan-400 mt-1">Result: {edu.result}</p>
                                                )}
                                            </div>
                                        </div>
                                    ))}
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        {/* Resume Card */}
                        <div className="rounded-2xl border border-slate-800 p-6 space-y-4 bg-slate-900/90 shadow-xl">
                            <h3 className="font-bold text-lg text-white flex items-center gap-2">
                                <FileText className="h-5 w-5 text-cyan-400" /> Curriculum Vitae
                            </h3>
                            ...
                            <div className="pt-2">
                                <a
                                    ...
                                    className="w-full inline-flex items-center justify-center px-4 py-3 text-sm font-semibold text-slate-950 bg-cyan-400 hover:bg-cyan-300 rounded-xl shadow-lg shadow-cyan-500/20 transition-all gap-2"
                                >
                        ...
                        {certificates && certificates.length > 0 && (
                            <div className="space-y-4">
                                <h3 className="font-bold text-lg text-white flex items-center gap-2">
                                    <Award className="h-5 w-5 text-indigo-400" /> Certifications
                                </h3>
