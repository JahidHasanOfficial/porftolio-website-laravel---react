...
export default function Edit({ blog }) {
    const { data, setData, processing, errors } = useForm({
        ...
        category: blog.category || 'Laravel',
        tags_text: blog.tags ? blog.tags.join(', ') : '',
    ...
    const handleDeleteBlog = () => {
        if (confirm(`Are you sure you want to permanently delete blog post "${blog.title}"?`)) {
            ...
            <div className="space-y-6 w-full">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-200 dark:border-slate-800">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 truncate max-w-md">{blog.title}</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 w-full">
                    ...
                    <div className="rounded-2xl border border-slate-200 dark:border-slate-805 bg-white dark:bg-slate-900 p-6 space-y-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="space-y-1.5">
                                ...
                                <select
                                    ...
                                    className="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 dark:border-slate-805 bg-slate-55 dark:bg-slate-955 text-slate-700 dark:text-slate-350 text-sm focus:outline-none focus:border-indigo-650 cursor-pointer"
                                >
                                    {categories.map((cat) => (
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-slate-500">Upload New Thumbnail (Optional)</label>
                            <input
                                type="file"
                            ...
                            {blog.thumbnail && (
                                <p className="text-[10px] text-slate-400 mt-1">Current: <a href={blog.thumbnail} target="_blank" className="underline">{blog.thumbnail}</a></p>
                            )}
                        </div>
                    </div>
